...
//echo $query_RSEdarat ;
    ...
    <div class="content CSSTableGenerator">
        ...
        <div class="button-group">
            ...
            <?php if ($pageNum_RSEdarat > 0) { // Show if not first page ?>
                <a title="السابق" class="button-primary" href="<?php printf("%s?pageNum_RSEdarat=%d%s", $currentPage, max(0, $pageNum_RSEdarat - 1), $queryString_RSEdarat); ?>"
                   tabindex="-1"> < </a>
            <?php } else { // Show if not first page ?>
            ...
            <?php if ($pageNum_RSEdarat < $totalPages_RSEdarat) { // Show if not last page ?>
                <a title="التالي" class="button-primary"
                   href="<?php printf("%s?pageNum_RSEdarat=%d%s", $currentPage, min($totalPages_RSEdarat, $pageNum_RSEdarat + 1), $queryString_RSEdarat); ?>" tabindex="-1"> > </a>
            <?php } else { // Show if not first page ?>
                <a title="التالي" class="button-primary is-disabled" href="#" tabindex="-1"> > </a>
            <?php } // Show if not last page ?>

            <?php if ($pageNum_RSEdarat < $totalPages_RSEdarat) { // Show if not last page ?>
                <a title="الصفحة الأخيرة" class="button-primary" href="<?php printf("%s?pageNum_RSEdarat=%d%s", $currentPage, $totalPages_RSEdarat, $queryString_RSEdarat); ?>"
                   tabindex="-1"> >> </a>
            <?php } else { // Show if not first page ?>
