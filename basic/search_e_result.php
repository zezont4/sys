...
$currentPage = '';
$name1 = "";
$name1 = Input::get('e_name1') ? "and name1 LIKE'%" . Input::get('e_name1') . "%'" : ' ';
$name2 = Input::get('e_name2') ? "and name2 LIKE'%" . Input::get('e_name2') . "%'" : ' ';
$name3 = Input::get('e_name3') ? "and name3 LIKE'%" . Input::get('e_name3') . "%'" : ' ';
$name4 = Input::get('e_name4') ? "and name4 LIKE'%" . Input::get('e_name4') . "%'" : ' ';
        ...
        <?php if (login_check($all_groups) == true) { ?>
            ...
            <center>
                <div class="button-group">
                    ...
                    <?php } ?>
                    <?php if ($page_no > 0) { // Show if not first page ?>
                        <a title="السابق" class="button-primary" href="<?php printf("%s?page_no=%d%s", $currentPage, max(0, $page_no - 1), $queryString_Rs_T); ?>" tabindex="-1">
                            < </a>
                    <?php } else { // Show if not first page ?>
                    ...
                    <?php if ($page_no < $totalPages_Rs_T) { // Show if not last page ?>
                        <a title="التالي" class="button-primary" href="<?php printf("%s?page_no=%d%s", $currentPage, min($totalPages_Rs_T, $page_no + 1), $queryString_Rs_T); ?>"
                           tabindex="-1"> > </a>
                    <?php } else { // Show if not first page ?>
                        <a title="التالي" class="button-primary is-disabled" href="#" tabindex="-1"> > </a>
                    <?php } // Show if not last page ?>
                    <?php if ($page_no < $totalPages_Rs_T) { // Show if not last page ?>
                        <a title="الصفحة الأخيرة" class="button-primary" href="<?php printf("%s?page_no=%d%s", $currentPage, $totalPages_Rs_T, $queryString_Rs_T); ?>"
                           tabindex="-1">
                            >> </a>
                    <?php } else { // Show if not first page ?>
