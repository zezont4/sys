...
$maxRows_Rs_T = 100;
$pageNum_Rs_T = 0;
if (isset($_GET['pageNum_Rs_T'])) {
  $pageNum_Rs_T = $_GET['pageNum_Rs_T'];
}
$startRow_Rs_T = $pageNum_Rs_T * $maxRows_Rs_T;

$T_Name1_Rs_T = "";
if (isset($_POST['t_name1'])) {
    $T_Name1_Rs_T = ($_POST['t_name1'] != "") ? "and TName1 LIKE'%" . $_POST['t_name1'] . "%'" : ' ';
}
$T_Name2_Rs_T = "";
if (isset($_POST['t_name2'])) {
    $T_Name2_Rs_T = ($_POST['t_name2'] != "") ? "and TName2 LIKE'%" . $_POST['t_name2'] . "%'" : ' ';
}
$T_Name3_Rs_T = "";
if (isset($_POST['t_name3'])) {
    $T_Name3_Rs_T = ($_POST['t_name3'] != "") ? "and TName3 LIKE'%" . $_POST['t_name3'] . "%'" : ' ';
}
$T_Name4_Rs_T = "";
if (isset($_POST['t_name4'])) {
    $T_Name4_Rs_T = ($_POST['t_name4'] != "") ? "and TName4 LIKE'%" . $_POST['t_name4'] . "%'" : ' ';
}
...
<?php include('../templates/header2.php'); ?>
        ...
        <table border="1" cellpadding="0" cellspacing="0">
            ...
            <?php do { ?>
                <tr <?php if ($row_Rs_T["hide"]==1){ ?> class='hidenRow' <?php ;} ?>>
                    ...
                    <td><?php echo $row_Rs_T['HName']; if($row_Rs_T['h_hide']==1){echo '<br>(الحلقة محذوفة)';}?></td>
    ...
    <center>
        <div class="button-group">
            ...
            <?php } ?>
            <?php if ($pageNum_Rs_T > 0) { // Show if not first page ?>
                <a title="السابق" class="button-primary" href="<?php printf("%s?pageNum_Rs_T=%d%s",$currentPage,max(0,$pageNum_Rs_T - 1),$queryString_Rs_T); ?>" tabindex="-1"> < </a>
                <?php }else{ // Show if not first page ?>
            ...
            <?php if ($pageNum_Rs_T < $totalPages_Rs_T) { // Show if not last page ?>
                <a title="التالي" class="button-primary" href="<?php printf("%s?pageNum_Rs_T=%d%s",$currentPage,min($totalPages_Rs_T,$pageNum_Rs_T + 1),$queryString_Rs_T); ?>" tabindex="-1"> > </a>
                <?php }else{ // Show if not first page ?>
                <a title="التالي" class="button-primary is-disabled" href="#" tabindex="-1"> > </a>
                <?php } // Show if not last page ?>
            <?php if ($pageNum_Rs_T < $totalPages_Rs_T) { // Show if not last page ?>
                <a title="الصفحة الأخيرة" class="button-primary" href="<?php printf("%s?pageNum_Rs_T=%d%s",$currentPage,$totalPages_Rs_T,$queryString_Rs_T); ?>" tabindex="-1"> >> </a>
                <?php }else{ // Show if not first page ?>
