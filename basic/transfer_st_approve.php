...
if (login_check("admin,t3lem") == true || $_token == 'STW6gPk8QrrZdF1gW2tO') {
    ...
    if (isset($_POST["transfer_id"])) {
        $update_transfer_note = sprintf("UPDATE `0_student_transfer` SET is_transferred=2,note=%s where id=%d",
            GetSQLValueString($_POST['cancel_reason'], 'text'),
            GetSQLValueString($_POST['transfer_id'], 'int'));
        $result_update_transfer = mysqli_query($localhost, $update_transfer_note) or die('$update_transfer_note : ' . mysqli_error($localhost));
    }
    ...
    if ((isset($_POST["mm_update"])) && ($_POST["mm_update"] == "form1")) {
        ...
        if ($total_rows_approve) {
            ...
            StEdarah=%s,StHalaqah=%s,`hide`=0,school_level=%s,nationality=%s WHERE StID=%s",
                ...
                $result_update_student = mysqli_query($localhost, $update_student) or die('$result_update_student : ' . mysqli_error($localhost));

                $update_transfer = sprintf("UPDATE `0_student_transfer` SET is_transferred=1 where StID=%s", $row_rs_approve['StID']);
                $result_update_transfer = mysqli_query($localhost, $update_transfer) or die('$result_update_transfer : ' . mysqli_error($localhost));
...
//    echo $query_rs_transfer;
    ...
    <div class="content CSSTableGenerator">
        <?php if ($total_rows_transfer) { ?>
            <table>
                ...
                <?php do { ?>
                    <tr id="tr_<?php echo $row_rs_transfer['id']; ?>" <?php echo $row_rs_transfer['old_name'] <> $row_rs_transfer['new_name'] ? 'class="different_name"' : ''; ?>>
                        ...
                        <td><a href="" onclick="cancel_transfer(this)">إلغاء</a></td>
                    </tr>
    ...
    <script>
        function cancel_transfer(a_link) {
            var parent_td = a_link.parentNode;
            var parent_tr_id = parent_td.parentNode.id;
            var transfer_id = parent_tr_id.replace("tr_", "");
            var form_already_exists = $('#new_tr_' + transfer_id).length;
            if (form_already_exists) {
                $('#new_tr_' + transfer_id).remove();
            } else {
                $("<tr id='new_tr_" + transfer_id + "'><td colspan='6'>" +
                    "<form action='#' method='post'>" +
                    "<input name='transfer_id' type='hidden' value='" + transfer_id + "'>" +
                    "<label for='cancel_reason'>سبب الرفض</label>" +
                    "<textarea style='background-color: #ffffff;width: 99%' name='cancel_reason' maxlength='255' rows='3'></textarea>" +
                    "<input type='submit' class='button-primary' value='رفض النقل'>" +
                    "</form>" +
                    "</td></tr>").insertAfter("#" + parent_tr_id);
            }
