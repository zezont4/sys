...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
    ...
    $query = sprintf("SELECT StID,StMobileNo FROM 0_students WHERE StID=%s",
        GetSQLValueString($_POST['StID'], "double"));
    $rs = mysqli_query($localhost, $query) or die('$query : ' . mysqli_error($localhost));
    $row = mysqli_fetch_assoc($rs);
    $rows_count = mysqli_num_rows($rs);
...
// تنبيه المستخدم لتحديد الجنسية لطلاب مرتقى الفاتحة
    ...
    $row = mysqli_fetch_assoc($rs);
    $rows_count = mysqli_num_rows($rs);
    if ($_POST['MurtaqaID'] >= 8 && ($row['nationality'] == '' || $row['nationality'] == NULL)) { ?>
        <?php include('../templates/header1.php'); ?>
        <br><br><br>
        <div style="direction:rtl;text-align:center;font-size:22px;padding: 10px;line-height: 46px">
            <p>
...
// تنبيه المستخدم لإضافة تاريخ المباشرة لطلاب مرتقى الفاتحة
    $query = sprintf("SELECT StID,StStartDate FROM 0_students WHERE StID=%s",
        GetSQLValueString($_POST['StID'], "double"));
    $rs = mysqli_query($localhost, $query) or die('$query : ' . mysqli_error($localhost));
    $row = mysqli_fetch_assoc($rs);
    $rows_count = mysqli_num_rows($rs);
    if ($_POST['MurtaqaID'] >= 8 && ($row['StStartDate'] == '' || $row['StStartDate'] == NULL)) { ?>
        <?php include('../templates/header1.php'); ?>
        <br><br><br>
        <div style="direction:rtl;text-align:center;font-size:22px;padding: 10px;line-height: 46px">
            <p>
                <?php
                echo implode([
                    'قبل حجز الموعد، يجب تحديد تاريخ مباشرة ',
                    get_gender_label('st', 'ال'),
                    ' ويقصد بالمباشرة أول يوم دراسة بالحلقات ولو كان في مجمع أو دار أخرى',
                    '<br>',
                    '<a href="../basic/student_edit.php?StID=' . $row['StID'] . '" >اضغط هنا لتعديل بياناته الأساسية</a>',
                ], ' ');
                ?>
...
## خاص بمن يرغب اختبار المرتقى الثامن بحيث يتأكد أن مدة دراسة الطالب تجاوزة السنتين
    $query_rs_fail = sprintf("SELECT StID,StStartDate FROM 0_students WHERE StID=%s",
        GetSQLValueString($_POST['StID'], "double"));
    ...
    if ($_POST['MurtaqaID'] >= 8 && $total_rows_rs_fail > 0) {

        $today = getHijriDate()->date;

        $date_diff = dateDiffHijri($row_rs_fail['StStartDate'], $today);

        $min_allowed_days = 720;
        if ($date_diff < $min_allowed_days) {
            ...
            <div style="direction:rtl;text-align:center;font-size:22px;padding: 10px;line-height: 46px">
                <p>
                    <?php echo implode(' ', [
                        'عفوا ، ',
                        get_gender_label('st', 'ال'),
                        ' لم يكمل/تكمل سنتين منذ الالتحاق بالجمعية ',
                        '<br>',
                        'ولا يمكنكم الحجز الآن لعدم إتمام سنتين منذ تاريخ المباشرة حسب لائحة المرتقيات الخاصة بالمرتقى الثامن',
                        '<br>',
                        'وسيسمح لكم بالحجز بعد',
                        $min_allowed_days - $date_diff,
                        'يوما / أيام وذلك يوم',
                        $day_name[getHijriDateAfter($today, $min_allowed_days - $date_diff)->day],
                        'الموافق',
                        StringToDate(getHijriDateAfter($today, $min_allowed_days - $date_diff)->date),
                        'هـ',
                        '<br>',
                        '<br>',
                        $note,
...
//	الدورات مطلوبة إذا كان المرتقى هو الفاتحة
        ...
        function fati7ah() {
            ...
            if ($("#MurtaqaID").val() == 9) {
                //خاص بدرجة الاختبار التجريبي
                $('#TestExamDegree').parsley('updateConstraint', {'range': [80, 100]});
                ...
                //خاص بدرجة الاختبار التجريبي
                $('#TestExamDegree').parsley('updateConstraint', {'range': [85, 100]});
...
<form action="<?php echo $editFormAction; ?>" method="post" name="form1" id="form1" data-validate="parsley">
    ...
    <div class="content">
        ...
        <div class="four columns omega">
            ...
            <input name="TestExamDegree" id="TestExamDegree" type="text" value="" data-required="true" data-type="digits" data-range="[80,100]"
                   autocomplete="off">
        </div>
        <div class="four columns alpha">
        ...
        <div class="eight columns alpha with_fati7ah">
            <div class="LabelContainer">الدورات التجويدية التي التحق بها الطالب/الطالبة</div>
