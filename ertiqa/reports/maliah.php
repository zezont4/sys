...
if (isset($_GET['Date1'])) {
    if ($_GET['Date1'] != null) {
        $Date1_Rs1 = 'and x.FinalExamDate>=' . str_replace('/', '', $_GET['Date1']);
    }
}
$Date2_Rs1 = "";
if (isset($_GET['Date2'])) {
    if ($_GET['Date2'] != null) {
        $Date2_Rs1 = 'and x.FinalExamDate<=' . str_replace('/', '', $_GET['Date2']);
    }
}
...
<?php require_once("../../templates/report_header2.php"); ?>
    <div class="reportContent">
        <p class="report_description">أمر صرف جوائز الارتقاء والبراعم من
            الفترة <?php echo(($_GET['Date1'] != '') ? $_GET['Date1'] : '(بداية النظام الإلكتروني في 1434/08/29 هـ)'); ?>
            إلى <?php echo(($_GET['Date2'] != '') ? $_GET['Date2'] : '(تاريخ اليوم)'); ?> </p>
...
//bra3m  #############################################
                ...
                if (isset($_GET['Date1'])) {
                    if ($_GET['Date1'] != null) {
                        $Date3_Rs1 = 'and DDate>=' . str_replace('/', '', $_GET['Date1']);
                    }
                }
                $Date4_Rs1 = "";
                if (isset($_GET['Date2'])) {
                    if ($_GET['Date2'] != null) {
                        $Date4_Rs1 = 'and DDate<=' . str_replace('/', '', $_GET['Date2']);
                    }
                }
                mysqli_select_db($localhost, $database_localhost);
