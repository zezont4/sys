    ...
    function recalculate() {
        ...
        for (var y = 1; y < 40; y++) {
            if ($('#m1' + y).length !== 0) {
                ...
                if ($('#hide_m1').is(':checked')) {m = 0;} else {m = n1;}
                if ($('#hide_m2').is(':checked')) {b = 0;} else {b = n2;}
                if ($('#hide_m3').is(':checked')) {tb = 0;} else {tb = n3;}
                if ($('#hide_m4').is(':checked')) {tb = 0;} else {e = n4;}
...
<?php require_once("../../templates/report_header2.php"); ?>
	<div class="reportContent">
		<p class="report_description">أمر صرف جوائز الارتقاء والبراعم من الفترة <?php echo (($_GET['Date1']!='') ? $_GET['Date1'] : '(بداية النظام الإلكتروني في 1434/08/29 هـ)') ; ?> إلى <?php echo  (($_GET['Date2']!='') ? $_GET['Date2'] : '(تاريخ اليوم)') ; ?> </p>
...
//bra3m  #############################################
	$Date3_Rs1 = "";
	if (isset($_GET['Date1'])) {
		if ($_GET['Date1']!=null) {$Date3_Rs1 = 'and DDate>=' . str_replace('/','',$_GET['Date1']);}
	}
	$Date4_Rs1 = "";
	if (isset($_GET['Date2'])) {
		if ($_GET['Date2']!=null) {$Date4_Rs1 = 'and DDate<=' . str_replace('/','',$_GET['Date2']);}
	}
	mysqli_select_db($localhost,$database_localhost);
	...
	<?php $i++; } while ($row_edarat = mysqli_fetch_assoc($edarat)); ?>
			<tr>
...
mysqli_free_result($rs_bra3m_money);
?>