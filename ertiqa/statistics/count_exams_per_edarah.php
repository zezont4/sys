...
if (isset($_GET['e_type'])) {
  if ($_GET['e_type']!=null) {$e_type =  'and e_type=' . $_GET['e_type'];}
}
	...
	<div class="FieldsTitle">تاريخ الاحصائية</div>
		...
		<div class="content CSSTableGenerator">
			...
			<table>
				...
				if ($s1 >= 80 && $s1 <= 100){$cls1='bg_green';}
				if ($s1 >= 60 && $s1 <= 79.99){$cls1='bg_yellow';}
				if ($s1 >= 0 && $s1 <= 59.99){$cls1='bg_red';}

				$s2=$row_Rs1['avg_degree'];
				if ($s2 >= 92 && $s2 <= 100){$cls2='bg_green';}
				if ($s2 >= 84 && $s2 <= 91.99){$cls2='bg_yellow';}
				if ($s2 >= 0 && $s2 <= 83.99){$cls2='bg_red';}
