    ...
    if ((isset($_POST["f_form1"])) && ($_POST["f_form1"] == "form1")) {
        ...
        if ($totalRows_all_f_teachers > 0) {
            do {
                ...
                <?php
                $updateSQL = sprintf("UPDATE ms_fahd_featured_teacher SET f_2a_t=%s,f_2b_t=%s,f_2a_n=%s,f_2a_d=%s,f_2b_n=%s,f_2b_d=%s,full_degree=%s WHERE auto_no=%s", $st_count, $st_count, $ertiqa_count, $new_ertiqa_degree, $new_bra3m_count, $new_bra3m_degree, $new_full_degree, $row_all_f_teachers['auto_no']);
                //echo $updateSQL.'<br>';
