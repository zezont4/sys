...
$default_year_name = isset($_SESSION['default_year_name']) ? $_SESSION['default_year_name'] : '';

$PageTitle = 'المسجلون في مسابقة المعلم المتميز';
if (login_check('admin,ms,t3lem,edarh') == true) {
    $editFormAction = $_SERVER['PHP_SELF'];
    if (isset($_SERVER['QUERY_STRING'])) {
        $editFormAction .= "?" . ($_SERVER['QUERY_STRING']);
    }

    $Date1_Rs1 = 'and f.f_t_date>=' . str_replace('/', '', $default_start_date);
    if (isset($_POST['Date1'])) {
        if (Input::get('Date1') != null) {
            $Date1_Rs1 = 'and f.f_t_date>=' . str_replace('/', '', Input::get('Date1'));
        }
    }
    $Date2_Rs1 = 'and f.f_t_date<=' . str_replace('/', '', $default_end_date);
    ...
    <div class="content">
        <div class="FieldsTitle">
            المسجلون في مسابقة المعلم
            المتميز <?php echo((Input::get('Date1') == '' && Input::get('Date2') == '') ? ' للعام الدراسي ( ' . $default_year_name . ' ) ' : ''); ?>
            خلال الفترة من <?php echo Input::get('Date1')  ? Input::get('Date1') : StringToDate($default_start_date . ' هـ '); ?>
            إلى <?php echo Input::get('Date2')  ? Input::get('Date2') : StringToDate($default_end_date) . ' هـ '; ?> </div>
