...
<?php if (login_check('admin,ms') == true) { ?>
    ...
    if (isset($_POST['Date1'])) {
        if ($_POST['Date1'] != null) {
            $Date1_Rs1 = 'and fhd.RDate>=' . str_replace('/', '', $_POST['Date1']);
        }
    }
    $Date2_Rs1 = 'and fhd.RDate<=' . str_replace('/', '', $_SESSION ['default_end_date']);
    if (isset($_POST['Date2'])) {
        if ($_POST['Date2'] != null) {
            $Date2_Rs1 = 'and fhd.RDate<=' . str_replace('/', '', $_POST['Date2']);
        }
    }

    mysqli_select_db($localhost, $database_localhost);
    $sql_sex = sql_sex('e.sex');
    $query_ReRegistered = sprintf("SELECT fhd.ErtiqaID,fhd.MsbkhID,fhd.SchoolLevelID,fhd.HalakahID,fhd.st_type,
...
<div class="FieldsTitle">
			المسجلون في مسابقة الفهد <?php echo(($_POST['Date1'] == '' && $_POST['Date2'] == '') ? ' للعام الدراسي ( ' . $_SESSION ['default_year_name'] . ' ) ' : ''); ?> خلال الفترة من <?php echo(($_POST['Date1'] != '') ? $_POST['Date1'] : StringToDate($_SESSION ['default_start_date']) . ' هـ '); ?> إلى <?php echo(($_POST['Date2'] != '') ? $_POST['Date2'] : StringToDate($_SESSION ['default_end_date']) . ' هـ '); ?></div>
