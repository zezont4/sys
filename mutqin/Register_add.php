...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
    ...
    $query_Rsdublicate = sprintf("SELECT AutoNo,StID,MsbkhID,RDate FROM `ms_mutqin_rgstr` WHERE `StID`=%s and MsbkhID = %s and RDate>=%s and RDate<=%s",
        GetSQLValueString($dublicate_RsSTID, "int"),
    ...
    if ($totalRows_Rsdublicate > 0) {
        ...
        echo '<h1 style="text-align:center;margin:20px;font-size:22px;">'
            ...
            '<br><br>' .
            '<a href="/sys/mutqin/Register_edit.php?auto_no=' . $row_Rsdublicate['AutoNo'] . '">' .
    ...
    $insertSQL = sprintf("INSERT INTO ms_mutqin_rgstr (EdarahID,HalakahID,TeacherID,StID, MsbkhID,SchoolLevelID,ErtiqaID,RDate) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)",
...
<?php $PageTitle = 'التسجيل في برنامج متقن'; ?>
...
if ($birthDate == null) {
    ...
    $b_date=null;
    if ($_SESSION['sex'] == 1) {
        $b_date = $mutqin_birth_date_b;
    } elseif ($_SESSION['sex'] == 0) {
        $b_date = $mutqin_birth_date_g;
    }
...
$open = false;
if ($_SESSION['sex'] == 1 && $mutqin_open_b == true) {
    $open = true;
} elseif ($_SESSION['sex'] == 0 && $mutqin_open_g == true) {
    $open = true;
...
if ($open == true) { ?>
    ...
    <div class="four columns">
        <div class="LabelContainer">الفرع</div>
        <?php echo create_combo("MsbkhID", $mutqin_msbkh_type, 0, '', 'class="full-width" data-required="true"'); ?>
    </div>
    ...
    <!-- closed -->
<?php } else {
    echo '<p class="WrapperMSG" >' . 'عفوا .. انتهت فترة التسجيل في برنامج متقن' . '</p>';
} ?>
<script>
