...
<?php $PageTitle = 'الطلاب المسجلون في مسابقة رعاية الشباب'; ?>
...
if (isset($_POST['Date1'])) {
  if ($_POST['Date1']!=null) {$Date1_Rs1 = 'and RDate>=' . str_replace('/','',$_POST['Date1']);}
}
$Date2_Rs1 = $Date2_Rs1 = 'and RDate<=' . $_SESSION ['default_end_date'];
if (isset($_POST['Date2'])) {
  if ($_POST['Date2']!=null) {$Date2_Rs1 = 'and RDate<=' . str_replace('/','',$_POST['Date2']);}
}
...
<div id="PageTitle"><?php echo $PageTitle; ?>
  ...
  * يمكنك الاستعلام بالتاريخ الأول فقط أو الثاني فقط أو بالجميع
		...
		<?php if (isset($Date1_Rs1)) { ?>
			<div class="FieldsTitle">المسجلون في مسابقة الاتقان <?php echo (($_POST['Date1']=='' && $_POST['Date2']=='') ? ' ( '.$_SESSION ['default_year_name'].' ) ': '' );?> خلال الفترة من <?php echo (($_POST['Date1']!='') ? $_POST['Date1'] : StringToDate($_SESSION ['default_start_date']).' هـ ') ; ?> إلى <?php echo  (($_POST['Date2']!='') ? $_POST['Date2'] : StringToDate($_SESSION ['default_end_date']).' هـ ') ; ?> </div>
			<?php } ?>
			<table border="1" cellpadding="0" cellspacing="0">
				<tr>
					...
					<?php if ($totalRows_ReRegistered>0) { // Show if recordset not empty ?>
						...
						<?php $i++; } while ($row_ReRegistered = mysqli_fetch_assoc($ReRegistered)); ?>
					<?php }else{?>
						<td colspan="7">لايوجد طلاب مسجلون في مسابقة رعاية الشباب حسب التاريخ المحدد</td>
					<?php }?>
			</table>
		</div>
		<div id="printButton">
